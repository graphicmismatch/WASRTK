...
class WASRTK {
  ...
  getReferenceApi() {
    return {
      ...
      setUserModified: (modified) => {
        userModifiedReference = modified;
      },
  ...
  setupEventListeners() {
    ...
    mainCanvas.addEventListener('mousedown', (e) => {
      ...
      if ((e.ctrlKey || e.metaKey) && referenceImage && referenceVisible) {
        ...
        if (
          mousePos.x >= referenceX &&
          mousePos.x <= referenceX + scaledWidth &&
          mousePos.y >= referenceY &&
          mousePos.y <= referenceY + scaledHeight
        ) {
    ...
    document
      ...
      .addEventListener('click', () => this.moveFrameLeft());
    document
      .getElementById('moveFrameRightBtn')
      .addEventListener('click', () => this.moveFrameRight());
    ...
    document
      ...
      .addEventListener('click', () => this.moveLayerDown());
    ...
    // Reference image
    document
      .getElementById('loadReferenceBtn')
      .addEventListener('click', () => this.loadReferenceImage());
    document
    ...
    document
      ...
      .addEventListener('click', () => this.hideModal('newProjectModal'));
  ...
  drawPoint(x, y, useStrokeCtx = false) {
    const useStrokeLayer =
      useStrokeCtx && strokeCtx && (currentTool === 'pen' || currentTool === 'eraser');
    const ctx = useStrokeLayer
      ? strokeCtx
      : (() => {
          const frame = frames[currentFrame];
          const layer = frame.layers[currentLayer];
  ...
  commitShape(start, end, tool) {
    ...
    if (antialiasingEnabled) {
      ...
      if (tool === 'line') {
        this.drawPixelPerfectLineWithFillRect(
          ctx,
          startCoords.x,
          startCoords.y,
          endCoords.x,
  ...
  moveLayerUp() {
    ...
    frames.forEach((frame) => {
      [frame.layers[currentLayer], frame.layers[currentLayer + 1]] = [
        frame.layers[currentLayer + 1],
        frame.layers[currentLayer],
      ];
    });
  ...
  updateStatusBar() {
    ...
    if (referenceImage && referenceVisible) {
      ...
      refStatus.title =
        'Reference image loaded. Ctrl+click to drag, Ctrl+/- to scale, Ctrl+R to re-center without changing zoom';
  ...
  async openFile(filePath) {
    try {
      ...
      if (supportedImageFormats.includes(fileExtension)) {
        ...
        throw new Error(
          `Unsupported file format: ${fileExtension}. Supported formats: ${supportedImageFormats.join(', ')}`
        );
      }
    } catch (error) {
  ...
  undo() {
    ...
    if (stateToRestore.type === 'draw') {
      ...
      const currentStateForRedo = ctxToSave.getImageData(
        0,
        0,
        canvasToSave.width,
        canvasToSave.height
  ...
  redo() {
    ...
    if (stateToRestore.type === 'draw') {
      ...
      const canvasToSave = layerToSave.canvas;
      const ctxToSave = canvasToSave.getContext('2d');
      const currentStateForUndo = ctxToSave.getImageData(
        0,
        0,
        canvasToSave.width,
        canvasToSave.height
