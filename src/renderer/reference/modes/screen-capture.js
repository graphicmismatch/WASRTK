...
function showScreenShareModal(app, sources) { const modal = document.createElement('div'); modal.className = 'modal show'; modal.id = 'screenShareModal';
  const modalContent = document.createElement('div'); modalContent.className = 'modal-content screen-share-modal';
  const title = document.createElement('h2'); title.textContent = 'Select Screen or Window';
  const sourcesContainer = document.createElement('div'); sourcesContainer.className = 'screen-sources';
  sources.forEach((source) => { const sourceItem = document.createElement('div'); sourceItem.className = 'screen-source-item'; sourceItem.innerHTML = `\n      <img src="${source.thumbnail}" alt="${source.name}" class="source-thumbnail">\n      <div class="source-info">\n        <span class="source-name">${source.name}</span>\n        <span class="source-type">${source.id.includes('screen') ? 'Screen' : 'Window'}</span>\n      </div>\n    `;
    sourceItem.addEventListener('click', () => { selectScreenSource(app, source); hideScreenShareModal(app); }); sourcesContainer.appendChild(sourceItem); });
  const cancelBtn = document.createElement('button'); cancelBtn.className = 'btn btn-secondary'; cancelBtn.textContent = 'Cancel'; cancelBtn.addEventListener('click', () => hideScreenShareModal(app));
  modalContent.appendChild(title); modalContent.appendChild(sourcesContainer); modalContent.appendChild(cancelBtn); modal.appendChild(modalContent); document.body.appendChild(modal);
}

function hideScreenShareModal(app) { const modal = document.getElementById('screenShareModal'); if (modal) modal.remove(); if (!app.screenCaptureInterval) {
  document.getElementById('screenShareBtn').innerHTML = '<i class="fas fa-desktop"></i>'; document.getElementById('screenShareBtn').title = 'Share Screen/Window'; document.getElementById('screenShareBtn').classList.remove('active');
}}

function selectScreenSource(app, source) {
  navigator.mediaDevices.getUserMedia({ audio: false, video: { mandatory: { chromeMediaSource: 'desktop', chromeMediaSourceId: source.id, minWidth: 800, maxWidth: 1920, minHeight: 600, maxHeight: 1080 } } }).then((stream) => {
    setupScreenShareStream(app, stream, source.name);
  }).catch((err) => { console.error('Error accessing screen:', err); tryAlternativeScreenCapture(app, source); });
}

function tryAlternativeScreenCapture(app, source) {
  navigator.mediaDevices.getUserMedia({ audio: false, video: { mandatory: { chromeMediaSource: 'desktop', chromeMediaSourceId: source.id } } }).then((stream) => {
    setupScreenShareStream(app, stream, source.name);
  }).catch((err) => { console.error('Alternative capture also failed:', err); alert('Screen capture failed. Please check your system permissions and try again.'); });
}
...
function setupScreenShareStream(app, stream, sourceName) {
  ...
  video.onerror = (err) => { console.error('Video error:', err); alert('Error setting up video stream. Please try again.'); };
}
